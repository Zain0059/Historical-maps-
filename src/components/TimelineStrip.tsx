...
export const TimelineStrip: React.FC<TimelineStripProps> = ({
  ...
  return (
    <nav id="timeline-bar" dir="rtl" aria-label="الشريط الزمني: الحقب ومراحلها"
      className="timeline-strip flex shrink-0 min-w-0 select-none z-30">
      {/* Navigation sits beside the two rows, without adding a third row. */}
      <div className="timeline-rows grid shrink-0 border-l border-[#a9863f]/40 bg-[#141c17]">
      ...
      <div ref={scrollContainerRef} dir="rtl"
        className="timeline-viewport flex flex-1 min-w-0 overflow-x-auto overscroll-x-contain">
        {eraGroups.map(({ era, entries }) => {
          ...
          return (
            <section key={era.id} aria-labelledby={headingId}
              className="shrink-0 timeline-rows grid border-l border-[#a9863f]/50">
              <h3 id={headingId} className="timeline-era-heading m-0 min-w-0 border-b border-[#a9863f]/40">
                <button onClick={() => onSelectSlide(isActiveEra ? currentIndex : entries[0].index)}
                  aria-current={isActiveEra ? 'true' : undefined}
                  className={`timeline-era-button h-full w-full text-start px-3 text-[12px] font-bold focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[#e9e0c7] ${isActiveEra ? 'bg-[#344032] text-[#fff1c9]' : 'bg-[#202c24] text-[#d9cfae] hover:bg-[#344032]'}`}
                  title={era.headline}>
                ...
                {entries.map(({ slide, index }, phaseIndex) => {
                  ...
                  return (
                    <button key={`${slide.id}-${slide.boundaryPhase?.id ?? 'era'}`}
                      ...
                      className={`timeline-stage h-full w-[156px] sm:w-[184px] shrink-0 px-2 py-1.5 border-l last:border-l-0 border-[#a9863f]/20 border-b-[3px] text-start transition-colors focus-visible:outline-2 focus-visible:outline-offset-[-3px] focus-visible:outline-[#e9e0c7] ${isActive ? 'bg-[#f0e3c2] border-b-[#b88041] text-[#241d12]' : 'border-b-transparent text-[#e9e0c7] hover:bg-[#2a382e]'}`}>
                      <span className="flex items-center gap-1.5 timeline-stage-date text-[11px] leading-4">
                      ...
                      <span className="line-clamp-2 text-[13px] font-medium leading-[17px] mt-1 whitespace-normal">{label}</span>
                    </button>
                  );
                })}
              </div>
      ...
      <div className="timeline-rows grid shrink-0 border-r border-[#a9863f]/40 bg-[#141c17]">
